...
class MultiHeadAttention(nn.Module):
    def __init__(self, heads, d_model, dropout = 0.1):
        ...
        self.multi_head_attention = MultiheadAttention(
            d_model, heads, 
            dropout=0., bias=False, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None)
    ...
    def forward(self, wordgrid_qu,h,w,mask=None):
        ...
        att_output,att_output_weigths = self.multi_head_attention(wordgr_qu_key,wordgr_qu_query,wordgr_qu_value)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self,wordgrid_qu,h,w):
        ...
        wordgrid_qu = self.norm1(wordgrid_qu + self.dropout_1(att_output))

        ff_output = self.ff(wordgrid_qu)
        wordgrid_qu = self.norm2(wordgrid_qu + self.dropout_2(ff_output))
...
class Encoder(nn.Module):
    ...
    def forward(self, image, question, question_len, embeddings, bboxes, emb_lengths):  # this is an image blind example (as in section 4.1)
        ...
        for i in range(self.n_layer):
            ...
            encoder_output,h,w = self.layers[i](encoder_input,h,w)
        ...
        output_wordgrid,question_output = torch.split(encoder_output,h*w,1)
